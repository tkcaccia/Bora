...
The primary raster output is `[sample]_refined.ome.tif`, containing the level-0 integer label map and eight successively downsampled levels for the current whole-slide configuration. Bora writes tiled pyramidal OME-TIFF with lossless LZW compression and `SIMPLE` nearest-neighbour downsampling so categorical label IDs are not interpolated. The companion GeoJSON is generated after pyramid conversion. The retained CellPhenotyper converter (Git blob `fd2dd40`) is available as a reference backend. Bora's default four-CPU backend polygonizes a selected categorical pyramid level, filters small components, simplifies the complete polygon coverage jointly so shared class boundaries remain coincident, dissolves components per label in parallel, and maps coordinates to level 0. It emits the theoretical minimum of one top-level feature per observed foreground label while retaining disconnected polygon parts and holes required for fidelity. Prespecified accurate, balanced, and fast profiles control pyramid level, area filtering, and simplification. Raster round-trip pixel accuracy, foreground IoU, per-label IoU, polygon parts, vertices, file size, wall time, and peak memory are reported.
