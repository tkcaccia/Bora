...
The primary raster output is `[sample]_refined.ome.tif`, containing the level-0 integer label map and eight successively downsampled levels for the current whole-slide configuration. Bora writes tiled pyramidal OME-TIFF with lossless LZW compression and `SIMPLE` nearest-neighbour downsampling so categorical label IDs are not interpolated. The companion GeoJSON is generated after pyramid conversion by an unchanged copy of CellPhenotyper's post-refinement `mask_to_geojson.py` (Git blob `fd2dd40`). Its default configuration selects the first pyramid level no larger than 2,048 pixels, maps coordinates back to level 0, dissolves regions by label value, drops regions below 500 level-0 pixel squared, performs three 10-pixel buffer/unbuffer smoothing passes, simplifies by 6 pixels with topology preservation, and removes holes. Each feature records a numeric `value` and CellPhenotyper-compatible `classification`. Because this representation is deliberately smoothed and hole-filled rather than lossless, raster round-trip agreement and per-label IoU must be reported alongside file size and geometry validity.
